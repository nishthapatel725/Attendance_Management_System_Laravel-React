...
const BarChart = () => {
  ...
  const [data, setData] = useState([20, 30, 40, 20, 45, 50, 30]); // Default data for the chart

  // Fetch semester names from API
  useEffect(() => {
    const fetchSemesters = async () => {
      try {
        const response = await fetch("http://127.0.0.1:8000/api/semester");
        const result = await response.json();
        const semesterNames = result.map((semester) => semester.name); // Extract names
